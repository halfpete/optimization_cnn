...
//Global Variables
unsigned long timestamp0; //Conv Layer1 Time 
unsigned long timestamp1; //Relu Layer1 Time
unsigned long timestamp2; //Pool Layer1 Time
unsigned long timestamp3; //Conv Layer2 Time
unsigned long timestamp4; //Relu Layer2 Time
unsigned long timestamp5; //Pool Layer2 Time
unsigned long timestamp6; //Conv Layer3 Time
unsigned long timestamp7; //Relu Layer3 Time
unsigned long timestamp8; //Pool Layer3 Time
unsigned long timestamp9; //Fc Layer Time
unsigned long timestamp10; //Softmax Layer Time
    ...
    for(int d = 0; d < l->out_depth; d++) {
      ...
      if (f->depth == 3) {
            ...
            for(int fy = 0; fy < f->sy; fy++) {
              ...
              for(int fx = 0; fx < f->sx; fx ++) {
                ...
                if(oy >= 0 && oy < V_sy && ox >=0 && ox < V_sx) {
                  a += f->w[((f->sx * fy)+fx)*3] * V->w[((V_sx * oy)+ox)*V->depth];
                  a += f->w[((f->sx * fy)+fx)*3+1] * V->w[((V_sx * oy)+ox)*V->depth+1];
                  a += f->w[((f->sx * fy)+fx)*3+2] * V->w[((V_sx * oy)+ox)*V->depth+2];
                }
              }
            }
          a += l->biases->w[d];
        ...
        for(int ay = 0; ay < l->out_sy; y += xy_stride, ay++) {
          ...
          for(int ax=0; ax < l->out_sx; x += xy_stride, ax++) {
            sum = 0.0;
            split_sum = _mm256_setzero_pd();
            for(int fy = 0; fy < f->sy; fy++) {
              oy = y + fy;
              for(int fx = 0; fx < f->sx; fx ++) {
                ox = x + fx;
                if(oy >= 0 && oy < V_sy && ox >=0 && ox < V_sx) {
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*16]);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*16]+4);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]+4);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*16]+8);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]+8);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*16]+12);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]+12);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);                                    
                  }
                }
          ...
          for(int ax=0; ax < l->out_sx; x += xy_stride, ax++) {
            ...
            for(int fy = 0; fy < f->sy; fy++) {
              ...
              for(int fx = 0; fx < f->sx; fx ++) {
                ox = x + fx;
                if(oy >= 0 && oy < V_sy && ox >=0 && ox < V_sx) {
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*20]);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*20]+4);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]+4);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*20]+8);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]+8);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*20]+12);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]+12);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);
                  b = _mm256_loadu_pd(&f->w[((f->sx * fy)+fx)*20]+16);
                  c = _mm256_loadu_pd(&V->w[((V_sx * oy)+ox)*V->depth]+16);
                  a = _mm256_mul_pd(b, c);
                  split_sum = _mm256_add_pd(a, split_sum);                          
                }
              }
...
void net_forward(network_t* net, batch_t* v, int start, int end) {
  unsigned long temp_timestamp = timestamp_us();
  conv_forward(net->l0, v[0], v[1], start, end);
  timestamp0 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  relu_forward(net->l1, v[1], v[2], start, end);
  timestamp1 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  pool_forward(net->l2, v[2], v[3], start, end);
  timestamp2 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  conv_forward(net->l3, v[3], v[4], start, end);
  timestamp3 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  relu_forward(net->l4, v[4], v[5], start, end);
  timestamp4 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  pool_forward(net->l5, v[5], v[6], start, end);
  timestamp5 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  conv_forward(net->l6, v[6], v[7], start, end);
  timestamp6 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  relu_forward(net->l7, v[7], v[8], start, end);
  timestamp7 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  pool_forward(net->l8, v[8], v[9], start, end);
  timestamp8 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();  
  fc_forward(net->l9, v[9], v[10], start, end);
  timestamp9 += timestamp_us() - temp_timestamp;
  temp_timestamp = timestamp_us();
  softmax_forward(net->l10, v[10], v[11], start, end);
  timestamp10 += timestamp_us() - temp_timestamp;
}
